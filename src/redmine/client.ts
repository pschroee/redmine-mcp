...
export class RedmineClient {
  ...
  async addIssueWatcher(issueId: number, userId: number): Promise<RedmineResult<void>> {
    return this.request<void>("POST", `/issues/${issueId}/watchers.json`, {
      user_id: userId,
    });
  }

  async removeIssueWatcher(issueId: number, userId: number): Promise<RedmineResult<void>> {
